...
4. `full` 采集完整 step；`block` 采集不含层内 hook 的完整目标层并保留 compile；`region` 以 eager 方式采集层内自然边界、op JSONL 和文本报告。
...
8. profile 环境变量必须在 Python 进程启动前设置；依赖导入期裁剪的 region hook 不能通过进程内后改环境变量启用。
9. profiler 输出用于性能诊断，不作为 bitwise 数值或画质回归基准；正确性回归必须关闭 profiler 独立运行。
10. 未知 GPU 不得阻断报告，但不能猜测 peak TFLOPS。
11. 稀疏算子的 FLOPs 必须注明口径；当前动态稀疏 attention 使用 `dense-equivalent`。
12. 不提交 `prof_results/`、模型、媒体结果、个人路径或临时运行配置。
...
| 文件 | 职责 |
|---|---|
| `lightx2v/utils/transformer_profile.py` | env 解析、目标校验、一次性 full/block/region 生命周期、输出目录 |
| `lightx2v/utils/region_profile.py` | 仅在 `region` 模式目标层激活的 region 与 active model profile；其他模式装饰器返回原函数 |
...
| model class | canonical config / script | full | block | region | 最新验证状态 |
|---|---|---:|---:|---:|---|
| `minimax_h3` | `configs/minimax_h3/dmd/minimax_h3_fp8_4step_5090_with_fp8_vae_sla.json` / `scripts/minimax_h3/run_minimax_h3_t2av_sp.sh` | 是 | 是，支持 compile 与 SP/TP，并输出 runtime inventory | 是，单卡 eager，含 AdaLN/self-attention/dense FFN | 2026-08-28 基于 RTX 5090 完成无 profile 的 SP8+compile、SP8 block layer 47 和单卡 region layer 47 E2E；单测覆盖 TP wrapper、AdaLN cache 与 LoRA FLOPs |
| `hunyuan3d` | `configs/hunyuan3d/hunyuan3d_shape.json` / `scripts/hunyuan3d/run_hunyuan3d.sh` | 是 | 是，完整层 trace | 是，含 self/cross attention、dense FFN、MoE | 已适配最新 main 并静态编译；本机缺模型，尚未重跑 E2E |
| `seko_talk` | `configs/seko_talk/seko_talk_01_base.json` / `scripts/seko_talk/run_seko_talk_01_base.sh` | 是 | 是，完整层 trace | 是，含 audio adapter | 已适配最新 main，Wan 回归测试通过；本机缺模型，尚未重跑 E2E |
| `wan2.2_moe_distill` | `configs/wan22/extreme/wan_moe_t2v_distill_nvfp4_sparse_attn.json` / `scripts/wan22/extreme/run_wan22_moe_t2v_extreme.sh` | 是 | 是，完整层 trace | 是，含 NVFP4、dynamic sparse attention、text cross-attention | 2026-07-24 在 RTX 5090 单卡完成 full 与原细粒度单层 E2E；本次拆分后需按最低验证重跑 block/region |
...
4. 再接 `block`：在统一的 `run_block` 边界绑定真实权重/输入 shape，并用 `record_block()` 包住原调用；必须继续走原有 compiled block。
5. 最后接 `region`：只在该模式安装或启用层内 hook，并明确拒绝不兼容的 compile/layout；不得为追求报告细粒度拆散核心推理结构。
...
3. `block` 模式，确认生成完整 compiled layer trace；已实现 inventory 的模型同时检查 runtime shape/FLOPs。
4. `region` 模式，确认 eager 配置生成 trace、op JSONL 和文本报告。
5. 检查目标 step/layer、主要 token 数、GEMM K/N、region assigned 比例和核心 kernel 配对。
6. 对量化、稀疏、MoE、adapter 等特例检查 FLOPs 口径，不只检查文件存在。
