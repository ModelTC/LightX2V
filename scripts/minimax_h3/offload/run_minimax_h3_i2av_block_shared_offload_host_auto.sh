...
config_template="$(absolute_path "${MINIMAX_H3_CONFIG:-${lightx2v_path}/configs/minimax_h3/offload/minimax_h3_t2av_block_shared_offload_host_auto.json}")"
...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
  ...
  --prompt "${MINIMAX_H3_PROMPT:-Animate this image with natural synchronized sound.}" \
  --image_path "$(absolute_path "${MINIMAX_H3_IMAGE_PATH:-${lightx2v_path}/assets/inputs/imgs/img_0.jpg}")" \
  --save_result_path "$(absolute_path "${MINIMAX_H3_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/minimax_h3_i2av_block_shared_offload_host.mp4}")" \
  --seed 42
