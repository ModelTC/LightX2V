...
exec python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node=8 -m lightx2v.infer \
  ...
  --config_json "${MINIMAX_H3_CONFIG:-${lightx2v_path}/configs/minimax_h3/offload/minimax_h3_t2av_block_offload_sp8.json}" \
  ...
  --save_result_path "${MINIMAX_H3_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/minimax_h3_t2av_block_offload_sp8.mp4}" \
  --seed 42
