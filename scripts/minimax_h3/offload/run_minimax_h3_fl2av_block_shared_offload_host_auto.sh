...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
  ...
  --image_path "$(absolute_path "${MINIMAX_H3_IMAGE_PATH:-${lightx2v_path}/assets/inputs/imgs/flf2v_input_first_frame-fs8.png}")" \
  --last_frame_path "$(absolute_path "${MINIMAX_H3_LAST_FRAME_PATH:-${lightx2v_path}/assets/inputs/imgs/flf2v_input_last_frame-fs8.png}")" \
  --save_result_path "$(absolute_path "${MINIMAX_H3_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/minimax_h3_fl2av_block_shared_offload_host.mp4}")" \
  --seed 42
