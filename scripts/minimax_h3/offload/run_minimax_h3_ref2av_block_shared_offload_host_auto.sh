...
reference_args=()
for kind in image video audio; do
    name="${kind}_path"
    [[ -n "${!name}" ]] || continue
    ...
    reference_args+=("--${kind}_path" "$(IFS=','; echo "${paths[*]}")")
done
...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
  ...
  "${reference_args[@]}" \
  --save_result_path "$(absolute_path "${MINIMAX_H3_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/minimax_h3_ref2av_block_shared_offload_host.mp4}")" \
  --seed 42
