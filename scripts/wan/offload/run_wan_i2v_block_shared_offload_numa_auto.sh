...
config_template="$(absolute_path "${CONFIG_JSON:-${lightx2v_path}/configs/offload/block/wan_i2v_block_shared_numa_auto.json}")"
...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
  ...
  --seed 42 \
  --save_result_path "$(absolute_path "${WAN_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/output_lightx2v_wan_i2v_block_shared_offload_numa.mp4}")"
