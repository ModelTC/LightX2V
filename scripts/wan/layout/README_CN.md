...
通用存储和传输逻辑在 `lightx2v/common/offload/block_layout.py`，Wan 加载约束在 `lightx2v/models/networks/wan/weights/block_layout.py`。连续布局方案支持单卡 Wan2.1 I2V、普通 CPU block offload 和 NoCaching；CUDA 支持原生浮点和 FP8-vLLM，Ascend 支持原生浮点和 INT8-NPU。共享权重、lazy load、并行、LoRA、CUDA Graph 等组合不在当前支持范围。
