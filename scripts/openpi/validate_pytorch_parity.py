...
def main() -> None:
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("/data/liuhongda/openpi_data/openpi-assets/checkpoints/pi05_libero_pytorch"),
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("/data/liuhongda/LightX2V/configs/openpi/pi05_libero.json"),
    )
    parser.add_argument(
        "--sample",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/data/liuhongda/openpi_data/results/pi05_libero_pytorch_parity.json"),
    )
    parser.add_argument("--device", default="cuda")
    args = parser.parse_args()

    image, wrist, state = load_sample(args.sample)
    ...
    package.__path__ = ["/data/liuhongda/LightX2V/lightx2v"]
