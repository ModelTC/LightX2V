...
config_template="$(absolute_path "${CONFIG_JSON:-${lightx2v_path}/configs/qwen_image/offload/qwen_image_t2i_2512_block_shared_numa_auto.json}")"
...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
  ...
  --negative_prompt " " \
  --save_result_path "$(absolute_path "${QWEN_SAVE_RESULT_PATH:-${lightx2v_path}/save_results/qwen_image_t2i_2512_block_shared_offload_numa.png}")" \
  --seed 42
