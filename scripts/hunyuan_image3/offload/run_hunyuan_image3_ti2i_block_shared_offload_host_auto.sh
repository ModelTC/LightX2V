...
if not os.environ.get("CONFIG_JSON"):
    ...
    config["flashinfer_autotune_cache"] = f"save_results/hunyuan_image3_flashinfer_autotune_ti2i_tp{tp}_sp{sp}_cfg{cfg}.json"
...
python -m torch.distributed.run --standalone --nnodes=1 --nproc-per-node="${nproc}" -m lightx2v.infer \
    ...
    --image_path "$(absolute_path "${HUNYUAN_IMAGE3_IMAGE_PATH:-${HUNYUAN_IMAGE3_REPO_PATH}/assets/demo_instruct_imgs/input_0_0.png}")" \
