...
# Usage:
#   bash scripts/hunyuan_image3/run_hunyuan_image3_ti2i_tp_sp_cfg_flashinfer.sh
#   bash scripts/hunyuan_image3/run_hunyuan_image3_ti2i_tp_sp_cfg_flashinfer.sh ti2iv2
#   bash scripts/hunyuan_image3/run_hunyuan_image3_ti2i_tp_sp_cfg_flashinfer.sh 0,1,2,3,4,5,6,7 ti2iv2
...
if [ "$DEMO" != "ti2i" ] && [ "$DEMO" != "ti2iv2" ]; then
    echo "DEMO must be either ti2i or ti2iv2, got: $DEMO" >&2
    exit 2
fi
...
config_json="${CONFIG_JSON:-${lightx2v_path}/configs/hunyuan_image3/hunyuan_image3_ti2i_tp_sp_cfg_flashinfer.json}"

parallel_values=$(python - "$config_json" "$model_path" "ti2i" <<'PY'
import json
import sys
...
if not config.get("enable_cfg", False) or cfg_size != 2 or cfg_mode != "parallel":
    raise SystemExit(
        f"{task} CFG launcher requires enable_cfg=true, cfg_p_size=2, and cfg_mode='parallel'; "
        f"got enable_cfg={config.get('enable_cfg', False)}, cfg_p_size={cfg_size}, cfg_mode={cfg_mode!r}."
    )
...
if [ "$DEMO" = "ti2iv2" ]; then
    ...
    default_seed=43
    default_save_result_path="${lightx2v_path}/save_results/hunyuan_image3_ti2iv2_tp2_sp2_cfg2_flashinfer.png"
else
    ...
    default_seed=42
    default_save_result_path="${lightx2v_path}/save_results/hunyuan_image3_ti2i_tp2_sp2_cfg2_flashinfer.png"
fi

prompt="${prompt:-$default_prompt}"
image_path="${image_path:-$default_image_path}"
seed="${seed:-$default_seed}"
save_result_path="${SAVE_RESULT_PATH:-${save_result_path:-$default_save_result_path}}"
