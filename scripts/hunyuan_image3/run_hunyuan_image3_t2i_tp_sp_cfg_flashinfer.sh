...
# Usage:
#   bash scripts/hunyuan_image3/run_hunyuan_image3_t2i_tp_sp_cfg_flashinfer.sh
#   bash scripts/hunyuan_image3/run_hunyuan_image3_t2i_tp_sp_cfg_flashinfer.sh 0,1,2,3,4,5,6,7
...
config_json="${CONFIG_JSON:-${lightx2v_path}/configs/hunyuan_image3/hunyuan_image3_t2i_tp_sp_cfg_flashinfer.json}"

parallel_values=$(python - "$config_json" "$model_path" "t2i" <<'PY'
import json
import sys
...
seed="${seed:-42}"
save_result_path="${SAVE_RESULT_PATH:-${save_result_path:-${lightx2v_path}/save_results/hunyuan_image3_t2i_tp2_sp2_cfg2_flashinfer.png}}"

echo "Using CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "Running HunyuanImage3 t2i with TP_SIZE=${TP_SIZE}, SP_SIZE=${SP_SIZE}, CFG_SIZE=${CFG_SIZE}, SP_ATTN_TYPE=${SP_ATTN_TYPE}"
