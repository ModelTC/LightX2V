...
```
使用单张H100，运行时间及使用`watch -n 1 nvidia-smi`观测的峰值显存参考如下：
...
3. 步数蒸馏模型 merge Lora：出现OOM
4. 步数蒸馏+FP8量化模型：Total Cost cost 25.430433 seconds；34218MiB


#### 1.2 单卡offload推理

如下修改 config 文件中的 cpu_offload，开启offload推理
```
    "cpu_offload": true,
...
# 方式1：多卡运行
# PROFILING_DEBUG_LEVEL=2 PYTHONPATH=/home/uesr/LightX2V torchrun --nproc_per_node=8 wan_t2v.py
