...
```
`cpu_offload` 表示是否启用cpu offload，启用cpu offload能达到降低显存的效果，若是开启cpu offload，则需要加上`"offload_granularity": "block"` 表示按 DiT 的 Transformer Block 为单位卸载。开启之后可以使用`watch -n 1 nvidia-smi` 观察显存使用情况。
...
```
`denoising_step_list` 表示 4 步推理对应的去噪强度
