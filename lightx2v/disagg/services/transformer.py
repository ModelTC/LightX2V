...
class TransformerService(BaseService):
    ...
    def init(self, config):
        ...
        if not isinstance(remote_room, dict):
            raise RuntimeError(
                f"remote transformer output room init failed for room={data_bootstrap_room}; "
                "sidecar ownership is required to keep transfers alive during service reclaim"
            )
        ...
        def _load_phase1_meta(max_retries: int = 20, retry_sleep_s: float = 0.05) -> dict:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                missing_shape_keys = [
                    ...
                    if not isinstance(parsed.get(key), (list, tuple)) or len(parsed.get(key)) == 0
                ]
