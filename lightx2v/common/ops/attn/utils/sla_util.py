...
def _force_local_score_blocks(pooled_score, count):
    count = min(count, pooled_score.shape[-1])
    if count <= 0:
        return pooled_score

    q_blocks, k_blocks = pooled_score.shape[-2:]
    ...
    return pooled_score.scatter(-1, local_indices, torch.finfo(pooled_score.dtype).max)


def _expand_kv_heads(pooled_kblocks, num_q_heads):
    num_kv_heads = pooled_kblocks.shape[1]
    if num_q_heads == num_kv_heads:
        return pooled_kblocks

    assert num_q_heads % num_kv_heads == 0, f"Q heads ({num_q_heads}) must be divisible by KV heads ({num_kv_heads})"
    return pooled_kblocks.repeat_interleave(num_q_heads // num_kv_heads, dim=1)


def _select_block_lut(pooled_qblocks, pooled_kblocks, topk_ratio, topk, force_local_blocks=0):
    pooled_kblocks = _expand_kv_heads(pooled_kblocks, pooled_qblocks.shape[1])
    if topk is not None and topk > 16:
    ...
    topk = int(topk_ratio * k_blocks) if topk is None else topk
    # Short sequences still keep one key block to match the training router.
    topk = max(1, min(k_blocks, topk))
...
def get_block_lut(q, k, topk_ratio, BLKQ=64, BLKK=64, topk=None):
    ...
    return _select_block_lut(pooled_qblocks, pooled_kblocks, topk_ratio, topk)
...
def _get_block_lut_nhd(
    ...
    return _select_block_lut(pooled_qblocks, pooled_kblocks, topk_ratio, topk, force_local_blocks)
...
def get_block_lut_nhd(
    ...
    if topk is not None and topk > 16 and not force_local_blocks:
