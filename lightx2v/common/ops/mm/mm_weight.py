...
class MMWeightInt8IntelXpu(MMWeightQuantTemplate):
    ...
    Checkpoint weights are signed INT8 in PyTorch Linear layout ``[N, K]``
    with one FP32 dequantization scale per output channel. Activations remain
    BF16/FP16 at the Python boundary and are quantized rowwise inside the SYCL
    extension before the native oneDNN S8 x S8 matmul.
    """
    ...
    def apply(self, input_tensor):
        ...
        output = output.reshape(*original_shape, self.weight.shape[0])
        if self.has_lora_branch:
            output = output + self.apply_lora(input_2d).reshape_as(output)
        return output
