...
class SageAttn3FP4RollingKVCachePool(RollingKVCachePool):
    ...
    def __init__(
        ...
        self._k_cache_type = k_cache_type
        self._v_cache_type = v_cache_type
        self._N_alloc = cdiv(int(cache_size), self._BLK) * self._BLK
        super().__init__(
            num_layers,
            self._N_alloc,
            num_heads,
    ...
    def store_kv(
        ...
        if k.size(0) != chunk_len or v.size(0) != chunk_len:
            raise ValueError(f"SageAttn3FP4RollingKVCachePool.store_kv shape mismatch: chunk_len={chunk_len}, k={k.size(0)}, v={v.size(0)}.")
        if start_idx % self._V_GROUP != 0 or chunk_len % self._V_GROUP != 0:
            raise ValueError(f"SageAttn3 FP4 KV requires start/chunk aligned to {self._V_GROUP} (got start_idx={start_idx}, chunk_len={chunk_len}).")

        padded_len = cdiv(chunk_len, self._BLK) * self._BLK
    ...
    def k_cache(
        ...
        padded_len = cdiv(kv_len, self._BLK) * self._BLK
    ...
    def roll_window(
        ...
        if sink_tokens % self._V_GROUP != 0 or num_evicted % self._V_GROUP != 0:
            raise ValueError(f"SageAttn3 FP4 roll_window requires sink_tokens/num_evicted aligned to {self._V_GROUP} (got sink_tokens={sink_tokens}, num_evicted={num_evicted}).")
        ...
        self._v_fp4[layer_id, :, :, dst_start // 2 : dst_end // 2].copy_(self._v_fp4[layer_id, :, :, src_start // 2 : src_end // 2].clone())
        self._v_scale[layer_id, :, :, dst_start // 16 : dst_end // 16].copy_(self._v_scale[layer_id, :, :, src_start // 16 : src_end // 16].clone())
