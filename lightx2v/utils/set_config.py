...
def set_args2config(args):
    ...
    _hunyuan_image3_cli_override_keys = (
        "enable_kv_cache",
    ...
    config["_hunyuan_image3_cli_override_snapshot"] = {
        k: getattr(args, k, None) for k in _hunyuan_image3_cli_override_keys if hasattr(args, k) and getattr(args, k, None) is not None
    }
...
def auto_calc_config(config):
    ...
    if config["model_cls"] == "hunyuan_image3":
        ...
        config["enable_auto_image_size"] = bool(config.get("enable_auto_image_size", False))
        if config.get("hunyuan_image_size") not in (None, ""):
            config["image_size"] = config["hunyuan_image_size"]
        ...
        config.setdefault("attn_impl", config.get("attn_implementation", "sdpa"))
        config["attn_implementation"] = config.get("attn_impl", config.get("attn_implementation", "sdpa"))
        ...
        _hunyuan_image3_cli_override_snapshot = config.pop("_hunyuan_image3_cli_override_snapshot", None)
        if isinstance(_hunyuan_image3_cli_override_snapshot, dict):
            for k, v in _hunyuan_image3_cli_override_snapshot.items():
                if k == "hunyuan_image_size" and v == "":
                    continue
                config[k] = v
            if config.get("hunyuan_image_size") not in (None, ""):
                config["image_size"] = config["hunyuan_image_size"]
    ...
    config.pop("_hunyuan_image3_cli_override_snapshot", None)
