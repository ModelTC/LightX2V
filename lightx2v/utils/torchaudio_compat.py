...
def load(
    ...
    try:
        ...
        if isinstance(filepath, io.BytesIO):
            ...
            # Read with soundfile
            if num_frames > 0:
                waveform, sample_rate = sf.read(filepath, start=frame_offset, frames=num_frames, dtype="float32", always_2d=True)
            else:
                waveform, sample_rate = sf.read(filepath, start=frame_offset, dtype="float32", always_2d=True)
...
class Resample:
    ...
    def __call__(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(waveform_np.shape[0]):
            resampled = librosa.resample(waveform_np[i], orig_sr=self.orig_freq, target_sr=self.new_freq, res_type="kaiser_best")
