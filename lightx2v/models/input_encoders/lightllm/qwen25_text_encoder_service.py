...
class LightLLMServiceTextEncoder:
    ...
    def __init__(self, config, device=None):
        ...
        self.service_url = config.get("service_url", 
                                      config.get("lightllm_service_url", "http://localhost:8010"))
        self.timeout = config.get("service_timeout", 
                                  config.get("lightllm_service_timeout", 30))  # 超时时间（秒）
        self.retry_times = config.get("service_retry", 
                                      config.get("lightllm_service_retry", 3))  # 重试次数
    ...
    def load(self):
        ...
        # 加载 tokenizer
        tokenizer_path = self.config.get("qwen25vl_tokenizer_path", 
        ...
        if self.config["task"] == "i2i":
            ...
            processor_path = self.config.get("qwen25vl_processor_path",
    ...
    def _call_service(self, text: str, images: Optional[List[Image.Image]] = None) -> dict:
        ...
        if images is not None and len(images) > 0:
            ...
            if image_token_count != len(images):
                logger.warning(f"Image token count ({image_token_count}) != image count ({len(images)}), "
                             f"adjusting to match prompt")
    ...
    def infer(self, text: List[str], image_list: Optional[List] = None) -> Tuple:
        ...
        logger.debug(f"Converted hidden states: shape={hidden_states.shape}, dtype={hidden_states.dtype}, "
