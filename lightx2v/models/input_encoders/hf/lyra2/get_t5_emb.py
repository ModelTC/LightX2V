...
def load_model_dcp(model, ckpt_path):
    ...
    logger.info(str(set_model_state_dict(model, _state_dict, options=StateDictOptions(strict=False))))
    return model


def _sync_model_states(model, src: int = 0) -> None:
    """Broadcast model weights from rank `src` to all other ranks."""
...
def load_model_torch(model, ckpt_path):
    ...
    _sync_model_states(model, src=0)
    return model


def load_model_torch_cpu(model, ckpt_path):
    """CPU-only state_dict load to avoid any CUDA allocation during init."""
    if is_rank0():
