    ...
    def forward(
        ...
        if self.enable_mouse and mouse_condition is not None:
            ...
            if is_causal and kv_cache_mouse is not None:
                ...
                group_mouse = [
                    mouse_condition[:, self.vae_time_compression_ratio * (i - self.windows_size) + pad_t : i * self.vae_time_compression_ratio + pad_t, :] for i in range(num_frame_per_block)
                ]
            else:
            ...
            if is_causal:
                if kv_cache_mouse is None:
                    ...
                    padded_q = torch.cat([q, torch.zeros([q.shape[0], padded_length, q.shape[2], q.shape[3]], device=q.device, dtype=v.dtype)], dim=1)
                    padded_k = torch.cat([k, torch.zeros([k.shape[0], padded_length, k.shape[2], k.shape[3]], device=k.device, dtype=v.dtype)], dim=1)
                    ...
                    if (current_end > kv_cache_mouse["global_end_index"].item()) and (num_new_tokens + kv_cache_mouse["local_end_index"].item() > kv_cache_size):
                        ...
                        kv_cache_mouse["k"][:, sink_tokens : sink_tokens + num_rolled_tokens] = kv_cache_mouse["k"][
                            :, sink_tokens + num_evicted_tokens : sink_tokens + num_evicted_tokens + num_rolled_tokens
                        ].clone()
                        kv_cache_mouse["v"][:, sink_tokens : sink_tokens + num_rolled_tokens] = kv_cache_mouse["v"][
                            :, sink_tokens + num_evicted_tokens : sink_tokens + num_evicted_tokens + num_rolled_tokens
                        ].clone()
                        # Insert the new keys/values at the end
                        local_end_index = kv_cache_mouse["local_end_index"].item() + current_end - kv_cache_mouse["global_end_index"].item() - num_evicted_tokens
        ...
        if self.enable_keyboard and keyboard_condition is not None:
            ...
            if is_causal and kv_cache_keyboard is not None:
                ...
                group_keyboard = [
                    keyboard_condition[:, self.vae_time_compression_ratio * (i - self.windows_size) + pad_t : i * self.vae_time_compression_ratio + pad_t, :] for i in range(num_frame_per_block)
                ]
            else:
                keyboard_condition = self.keyboard_embed(keyboard_condition)
            ...
            if use_rope_keyboard:
                ...
                if is_causal:
                    if kv_cache_keyboard is None:
                        ...
                        if (current_end > kv_cache_keyboard["global_end_index"].item()) and (num_new_tokens + kv_cache_keyboard["local_end_index"].item() > kv_cache_size):
                            ...
                            kv_cache_keyboard["k"][:, sink_tokens : sink_tokens + num_rolled_tokens] = kv_cache_keyboard["k"][
                                :, sink_tokens + num_evicted_tokens : sink_tokens + num_evicted_tokens + num_rolled_tokens
                            ].clone()
                            kv_cache_keyboard["v"][:, sink_tokens : sink_tokens + num_rolled_tokens] = kv_cache_keyboard["v"][
                                :, sink_tokens + num_evicted_tokens : sink_tokens + num_evicted_tokens + num_rolled_tokens
                            ].clone()
                            # Insert the new keys/values at the end
                            local_end_index = kv_cache_keyboard["local_end_index"].item() + current_end - kv_cache_keyboard["global_end_index"].item() - num_evicted_tokens
                            local_start_index = local_end_index - num_new_tokens
                        else:
