...
def can_split_qkv_norm(packed, norm_q, norm_k, require_triton=True):
    ...
    return (
        (not require_triton or triton is not None)
...
def _split_qkv_norm_rope_fake(packed, q_weight, k_weight, cos, sin, q_eps, k_eps, low_precision_rope=False):
    return _split_qkv_norm_fake(packed, q_weight, k_weight, q_eps, k_eps)
...
def try_split_qkv_norm_rope(packed, norm_q, norm_k, rope, freqs, backend="triton"):
    ...
    if not can_split_qkv_norm(packed, norm_q, norm_k, require_triton=False) or packed.device.type not in ("cuda", "xpu"):
        return None
