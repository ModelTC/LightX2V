...
from lightx2v.models.networks.lyra2._imaginaire.lazy_config.file_io import PathManager
from lightx2v.models.networks.lyra2._imaginaire.lazy_config.registry import _convert_target_to_string
...
for _name, _fn in [("add", lambda *vals: sum(vals)), ("subtract", lambda *vals: vals[0] - sum(vals[1:]))]:
    try:
        OmegaConf.register_new_resolver(_name, _fn)
    except ValueError:
        pass  # resolver already registered (e.g. by lyra_2._ext.imaginaire.lazy_config)
