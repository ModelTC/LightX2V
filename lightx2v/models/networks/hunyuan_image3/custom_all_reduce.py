...
class HunyuanImage3CustomAllReduce:
    ...
    def initialize(self) -> bool:
        ...
        if self._initialization_attempted:
            if self.required and not self._available:
                raise HunyuanImage3CustomAllReduceError(f"Required HunyuanImage3 AR custom all-reduce is unavailable: {self._unavailable_reason}.")
    ...
    def capture(self) -> Iterator[None]:
        ...
        if not self.available or self._backend is None:
            if self.required:
                raise HunyuanImage3CustomAllReduceError(f"Required AR custom all-reduce is unavailable during graph capture: {self._unavailable_reason or 'not_initialized'}.")
            yield
            return
