...
class Head(nn.Module):
    def __init__(self, dim: int, out_dim: int, patch_size, eps: float):
        ...
        # 兼容 patch_size 为整数或元组
        if isinstance(patch_size, (tuple, list)):
            prod = math.prod(patch_size)
        else:
            prod = patch_size
        self.head = nn.Linear(dim, out_dim * prod)
...
class WanAudioModel(ModelMixin, ConfigMixin):
    ...
    def unpatchify(self, x: torch.Tensor, grid_size):
        f = int(grid_size[0]) if isinstance(grid_size, (tuple, list)) else int(grid_size)
        p = int(self.patch_size[0])
        return rearrange(
            x, 'b f (p c) -> b c (f p)',
            f=f,
            p=p
