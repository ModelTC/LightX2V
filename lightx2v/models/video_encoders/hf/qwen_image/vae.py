...
class AutoencoderKLQwenImageVAE:
    def __init__(self, config):
        ...
        self.model = AutoencoderKLQwenImage.from_pretrained(
            os.path.join(config.model_path, "vae")
        ).to(torch.device("cuda")).to(torch.bfloat16)
    ...
    def decode(self, latents):
        ...
        latents_mean = (
            torch.tensor(self.config.vae_latents_mean)
            .view(1, self.config.vae_z_dim, 1, 1, 1)
            .to(latents.device, latents.dtype)
        )
        latents_std = 1.0 / torch.tensor(self.config.vae_latents_std).view(1, self.config.vae_z_dim, 1, 1, 1).to(
