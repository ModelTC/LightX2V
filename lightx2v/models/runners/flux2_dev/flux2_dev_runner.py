...
from lightx2v.models.input_encoders.hf.flux2_dev.mistral3_model import Flux2Dev_TextEncoder
from lightx2v.models.networks.flux2_dev.model import Flux2DevTransformerModel
from lightx2v.models.runners.default_runner import DefaultRunner
from lightx2v.models.schedulers.flux2_dev.scheduler import Flux2DevScheduler
from lightx2v.models.video_encoders.hf.flux2_dev.vae import Flux2DevVAE
...
class Flux2DevRunner(DefaultRunner):
    ...
    def init_modules(self):
        logger.info("Initializing Flux2Dev modules...")
        self.load_model()
    ...
    def run_text_encoder(self, text, image_list=None):
    ...
    def get_custom_shape(self):
        ...
        if len(self.input_info.target_shape) == 2:
            ...
            logger.info(f"Flux2Dev Image Runner got custom shape: {width}x{height}")
            return (width, height)

        if self.input_info.aspect_ratio and not self.config.get("_auto_resize", False):
            if self.input_info.aspect_ratio in as_maps:
                logger.info(f"Flux2Dev Image Runner got aspect ratio: {self.input_info.aspect_ratio}")
