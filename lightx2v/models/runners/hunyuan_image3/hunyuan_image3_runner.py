...
class HunyuanImage3Runner(DefaultRunner):
    ...
    def _resolve_image_size(self, input_info):
        hunyuan_image_size = self.config.get("hunyuan_image_size")
        if hunyuan_image_size not in (None, ""):
            return hunyuan_image_size
    ...
    def _fallback_image_size(self):
        return int(self.config.get("target_height", 1024)), int(self.config.get("target_width", 1024))

    def _needs_auto_image_size(self, image_size):
        return image_size == "auto" and bool(self.config.get("enable_auto_image_size", False))

    def _maybe_rewrite_prompt(self, prompt):
        if not bool(self.config.get("rewrite", False)):
            return prompt
        ...
        if not deepseek_key_id or not deepseek_key_secret:
            raise ValueError(
                "DeepSeek API key is not set. Set DEEPSEEK_KEY_ID and DEEPSEEK_KEY_SECRET "
                "or run with --rewrite false."
            )
        self._import_upstream_modules()
        DeepSeekClient = importlib.import_module("PE.deepseek").DeepSeekClient
        prompt_module = importlib.import_module("PE.system_prompt")
        prompt_name = self.config.get("sys_deepseek_prompt", "universal")
        if prompt_name == "universal":
            system_prompt = prompt_module.system_prompt_universal
        elif prompt_name == "text_rendering":
            system_prompt = prompt_module.system_prompt_text_rendering
        else:
            raise ValueError(f"Invalid sys_deepseek_prompt={prompt_name!r}.")
        rewritten_prompt, _ = DeepSeekClient(deepseek_key_id, deepseek_key_secret).run_single_recaption(system_prompt, prompt)
        logger.info(f"HunyuanImage3 rewrite prompt: {rewritten_prompt}")
    ...
    def _prepare_text_generation_inputs(self, prompt, bot_task, system_prompt, batch_cond_images=None, cond_inputs=None, cot_text=None):
    ...
    def _get_ratio_token_ids(self):
        tokenizer = self.hunyuan_tokenizer
        if hasattr(tokenizer, "get_all_ratio_token_ids"):
            return list(tokenizer.get_all_ratio_token_ids())
        return list(range(tokenizer.start_ratio_token_id, tokenizer.end_ratio_token_id + 1))

    def _get_ratio_index_from_token(self, ratio_token_id):
        tokenizer = self.hunyuan_tokenizer
        if hasattr(tokenizer, "get_all_ratio_token_ids"):
            ratio_token_ids = list(tokenizer.get_all_ratio_token_ids())
            try:
                ratio_index = ratio_token_ids.index(int(ratio_token_id))
            except ValueError as exc:
                raise ValueError(f"Unknown HunyuanImage3 ratio token id {ratio_token_id}") from exc
        else:
            ratio_index = int(ratio_token_id) - int(tokenizer.start_ratio_token_id)
        if ratio_index < 0 or ratio_index >= len(self.hunyuan_image_processor.vae_reso_group):
            raise ValueError(f"HunyuanImage3 ratio_index={ratio_index} is outside vae_reso_group.")
        return ratio_index

    def _generate_auto_image_size(self, prompt, cot_text=None, batch_cond_images=None, cond_inputs=None):
        prepared = self._prepare_text_generation_inputs(
            prompt,
            "img_ratio",
            self._resolve_system_prompt("img_ratio"),
            batch_cond_images=batch_cond_images,
            cond_inputs=cond_inputs,
            cot_text=cot_text,
        )
        ratio_tokens = self._get_ratio_token_ids()
        plan = HunyuanImage3TextGenerationPlan(first_bot_task="img_ratio", stage_transitions=[], final_stop_tokens=ratio_tokens)
        generated_tokens = self._generate_text_tokens(
            ...
            max_new_tokens=1,
            allowed_token_ids=ratio_tokens,
            force_greedy=True,
        )
        if not generated_tokens:
            raise RuntimeError("HunyuanImage3 auto image size generation produced no ratio token.")
        ratio_index = self._get_ratio_index_from_token(generated_tokens[-1])
        resolution = self.hunyuan_image_processor.vae_reso_group[ratio_index]
        image_size = (int(resolution.height), int(resolution.width))
        logger.info(f"HunyuanImage3 auto image size resolved to {image_size} by ratio_index={ratio_index}")
        return image_size

    def _mask_logits_to_allowed_tokens(self, logits, allowed_token_ids):
        if not allowed_token_ids:
            return logits
        allowed = torch.as_tensor(allowed_token_ids, dtype=torch.long, device=logits.device)
        masked = torch.full_like(logits, torch.finfo(logits.dtype).min)
        masked.index_copy_(dim=-1, index=allowed, source=logits.index_select(dim=-1, index=allowed))
        return masked

    def _sample_text_token(self, logits, generator, allowed_token_ids=None, force_greedy=False):
        do_sample = bool(self.config.get("text_do_sample", getattr(self.hunyuan_generation_config, "do_sample", False)))
        logits = logits.float()
        logits = self._mask_logits_to_allowed_tokens(logits, allowed_token_ids)
        if force_greedy or not do_sample:
    ...
    def _generate_text_tokens(
        ...
        for _ in range(max_new_tokens):
            if pending_tokens:
                ...
                next_token = self._sample_text_token(logits, generator, allowed_token_ids=allowed_token_ids, force_greedy=force_greedy)
    ...
    def _parse_float_sequence_config(self, key):
        ...
        raise TypeError(f"HunyuanImage3 {key} should be a comma-separated string or a list, got {type(value)}.")

    def _set_denoise_timesteps(self, num_steps, device):
        timesteps = self._parse_float_sequence_config("hunyuan_timesteps")
        sigmas = self._parse_float_sequence_config("hunyuan_sigmas")
        if timesteps is not None or sigmas is not None:
            self.scheduler.set_timesteps(num_steps, device=device, timesteps=timesteps, sigmas=sigmas)
        else:
            self.scheduler.set_timesteps(num_steps, device=device)

    def _rescale_noise_cfg(self, noise_cfg, noise_pred_text, guidance_rescale):
        ...
        noise_pred_rescaled = noise_cfg * (std_text / torch.clamp(std_cfg, min=1e-6))
    ...
    def _denoise_latents(self, prepared_inputs, image_size):
        ...
        self._set_denoise_timesteps(num_steps, latents.device)
        ...
        progress_iter = tqdm(
            ...
            disable=not bool(self.config.get("show_progress_bar", True)),
        )
        for step_index, timestep in enumerate(progress_iter):
            ...
            timestep_r_input = None
            if self.config.get("use_meanflow", False):
                if not hasattr(self.scheduler, "get_timestep_r"):
                    raise NotImplementedError("HunyuanImage3 meanflow requires scheduler.get_timestep_r.")
                timestep_r_input = self.scheduler.get_timestep_r(timestep).repeat(latent_model_input.shape[0])
            ...
            if self.config.get("use_meanflow", False):
                model_inputs["timesteps_r"] = timestep_r_input
            ...
            if prepared_inputs["do_cfg"]:
                ...
                if guidance_rescale > 0.0:
                    prediction = self._rescale_noise_cfg(prediction, pred_cond, guidance_rescale)

            latents = self.scheduler.step(prediction, timestep, latents, return_dict=False)[0]
            progress_callback = getattr(self, "progress_callback", None)
            if progress_callback:
                progress_callback(((step_index + 1) / total_steps) * 100, 100)
    ...
    def generate_t2i(self, input_info):
        ...
        prompt = self._maybe_rewrite_prompt(prompt)
        ...
        if self._needs_auto_image_size(image_size):
            image_size = self._generate_auto_image_size(prompt, cot_text=cot_text)
        elif image_size == "auto":
            image_size = self._fallback_image_size()
    ...
    def generate_i2i(self, input_info):
        ...
        requested_image_size = self._resolve_image_size(input_info)
        ...
        if self._needs_auto_image_size(requested_image_size):
            image_size = self._generate_auto_image_size(
                prompt,
                cot_text=cot_text,
                batch_cond_images=batch_cond_images,
                cond_inputs=text_cond_inputs,
            )
        else:
            image_size = self._resolve_ti2i_image_size(requested_image_size, batch_cond_images)
