...
def load_official_postprocess(source_path):
    ...
    if not (source_root / "inference" / "utils_3d").is_dir():
        raise FileNotFoundError(
            f"SenseNova-Vision source directory is invalid: {source_root}. "
            "Expected inference/utils_3d for GLB postprocessing."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "SenseNova recon3d GLB postprocessing requires the official source "
            "and optional packages open3d/trimesh/scipy. Raw .npy output is still available."
        ) from exc
...
def load_official_visualizers(source_path):
    ...
    if not init_path.is_file() or not visualize_path.is_file():
        raise FileNotFoundError(
            f"SenseNova-Vision source directory is invalid: {source_root}. "
            "Expected utils/visualize.py for official result visualization."
        )
