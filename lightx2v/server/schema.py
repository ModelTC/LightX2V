...
class VideoTaskRequest(BaseTaskRequest):
    ...
    src_ref_images: list[str] = Field(default_factory=list, description="VACE reference images as base64, URL, or server-local paths")
    prompt_ref: Optional[str] = Field(None, description="Prompt describing the driving video for Wan-Animate-2")
    src_pose_path: Optional[str] = Field(None, description="Server-local pose video path")
    src_face_path: Optional[str] = Field(None, description="Server-local face video path")
    src_bg_path: Optional[str] = Field(None, description="Server-local background video path")
    ...
    image_frame_idx: Optional[list[int]] = Field(None, description="LTX pixel frame index for each conditioning image")
...
class ImageTaskRequest(BaseTaskRequest):
    ...
    sr_ratio: float = Field(2.0, gt=0, description="Super-resolution scale factor used when target_shape is not set")
    keep_original_aspect: Optional[bool] = Field(None, description="Preserve a single HiDream reference image's aspect ratio")
    layout_bboxes: Optional[str] = Field(None, description="HiDream layout boxes as a JSON string or server-local JSON file path")
    infer_align_image_size: Optional[bool] = Field(None, description="Align HunyuanImage3 reference image sizes during inference")
...
class TaskRequest(BaseTaskRequest):
    target_video_length: Optional[int] = Field(None, description="Target video length; defaults to the startup config")
    audio_path: str = Field("", description="Input audio path (Wan-Audio)")
