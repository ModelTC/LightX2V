...
from .dmd.autoregressive_dmd import AutoregressiveDmdTrainer


@TRAINER_REGISTER("one_forcing")
class OneForcingTrainer(AutoregressiveDmdTrainer):
    ...
    def __init__(self, config):
        ...
        if self.gan_g_weight == 0 and self.gan_d_weight == 0:
            logger.warning("Both One-Forcing GAN weights are zero; training reduces to autoregressive_dmd.")
    ...
    def _real_latent(self, sample):
        ...
        return latent.to(device=self.student.device, dtype=self.latent_dtype)
    ...
    def forward_loss(self, latent_shape, conditions, real_latent, stage):
        ...
        noise = torch.randn(latent_shape, device=self.student.device, dtype=torch.float32)
        with torch.no_grad():
            renoised_xt = self.scheduler.add_noise(generated, noise, sigma)

        if stage == "fake":
            self.fake.set_training(True)
            velocity_fake = self._predict_velocity(self.fake, renoised_xt, sigma, condition)
