...
def parse_role_lora_config(
    ...
    if train_type not in {"full", "lora"}:
        raise ValueError(
            f"training.{role}.train_type must be 'full' or 'lora'."
        )
    if train_type == "full":
        return None
    ...
    def from_mapping(
        ...
        *,
        ...
        if dmd_config.get("cdm", {}).get("enabled", False):
            raise ValueError(
                "video_phased_dmd does not support training.dmd.cdm."
            )
        phased = dmd_config.get("phased", {})
        if not isinstance(phased, dict):
            raise ValueError(
                "training.dmd.phased must be a mapping."
            )
        match_timestep = int(phased.get("match_timestep", 500))
        if match_timestep not in base_config.denoising_step_list:
            raise ValueError(
                "training.dmd.phased.match_timestep must appear in "
                "training.dmd.denoising_step_list."
            )
        match_step_index = (
            base_config.denoising_step_list.index(match_timestep)
        )
        if not (
            0
            < match_step_index
            < len(base_config.denoising_step_list)
        ):
            raise ValueError(
                "training.dmd.phased.match_timestep must split "
                "training.dmd.denoising_step_list into non-empty High "
                "and Low regions."
            )
        ...
        if not isinstance(inference_steps, list):
            raise ValueError(
                "video_phased_dmd requires "
                "inference.denoising_step_list."
            )
        ...
        if inference_step_count != len(inference_steps):
            raise ValueError(
                "inference.num_inference_steps must equal the length "
                "of inference.denoising_step_list."
            )
        ...
        if infer_boundary != configured_boundary:
            raise ValueError(
                "inference.boundary_step_index must equal the position "
                "of training.dmd.phased.match_timestep in "
                "inference.denoising_step_list."
            )
        if "training_target" in phased:
            raise ValueError(
                "training.dmd.phased.training_target is no longer "
                "supported. Phased DMD always trains High and Low "
                "regions together."
            )
        if max_train_iters < 2:
            raise ValueError(
                "video_phased_dmd requires "
                "training.max_train_iters >= 2 so both High and Low "
                "regions are trained."
            )
        ...
        if not (
            ...
            raise ValueError(
                "training.dmd.phased score timestep bounds must "
                "satisfy 1 <= min < max <= "
                f"{base_config.num_train_timestep}."
            )
        if (
            score_min >= match_timestep
            or score_max < match_timestep + margin
        ):
            raise ValueError(
                "training.dmd.phased score timestep bounds must leave "
                "non-empty Low and High ranges around match_timestep."
            )
        if not (
            0
            < margin
            < base_config.num_train_timestep - match_timestep
        ):
            raise ValueError(
                "training.dmd.phased.score_timestep_margin must be "
                "positive and leave score timesteps above the phase "
                "boundary."
            )
        eps = float(phased.get("eps", 1.0e-8))
        norm_clip_min = float(
            dmd_config.get("norm_clip_min", 1.0e-4)
        )
        if eps <= 0 or norm_clip_min <= 0:
            raise ValueError(
                "training.dmd.phased.eps and "
                "training.dmd.norm_clip_min must be positive."
            )
        ...
        return cls(
            **base_config.__dict__,
            ...
            guidance_distill=float(
                phased.get("guidance_distill", 6.0)
            ),
            student_2=student_2,
            fake_2=fake_2,
            enable_fake_low_high=bool(
                phased.get("enable_fake_low_high", True)
            ),
            ...
            student_2_optimizer=copy.deepcopy(
                student_2["optimizer"]
            ),
            fake_2_optimizer=fake_2_optimizer,
            fake_low_high_optimizer=copy.deepcopy(
                fake_2_optimizer
            ),
        )
