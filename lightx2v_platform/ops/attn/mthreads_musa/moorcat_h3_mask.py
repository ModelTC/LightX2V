...
def normalize_condition_event_order(events, visual_count, audio_count):
    ...
    imgvid_indices = [
        index for item_type, index in events if item_type == "imgvid"
    ]
    ...
    audio_indices = [
        index for item_type, index in events if item_type == "audio"
    ]
...
def _cube_token_size(cube_size):
    ...
    if token_size & (token_size - 1):
        raise ValueError(
            "local_cube_size product must be a power of two for Moorcat, "
...
def _build_cube_segment_layout(
    ...
    if unassigned.numel():
        ...
        raise ValueError(
            f"packed token {first_token} in sample {first_sample} is not assigned to a stream"
        )
...
def precompute_cube_attention(
    ...
    base_block_counts, base_block_indices = _build_physical_base_layout(
        base_block_mask, block_labels
    )
        ...
        layout=CubeLayout(
            ...
            label_offsets_i32=torch.cat(
                (
                    ...
                    counts_per_label.to(torch.int32).cumsum(
...
def _cube_topk_selection(q_real, k_real, precomputed, topk_ratio):
    ...
    if (
        q_real.device.type in {"cuda", "musa"}
        and q_real.dtype is torch.bfloat16
        and k_real.dtype is torch.bfloat16
        and q_real.is_contiguous()
        and k_real.is_contiguous()
    ):
...
def cube_topk_block_indices(q_real, k_real, precomputed, topk_ratio):
    ...
    base_counts = torch.where(
        sparse_q_blocks.view(1, -1), base_counts, torch.ones_like(base_counts)
    )
    ...
    compact_source = torch.where(
        candidate_valid, candidate_indices, torch.zeros_like(candidate_indices)
    )
    ...
    if (
        buffer is None
        or tuple(buffer.shape[:-1]) != expected_prefix
        or buffer.shape[-1] < packed_width
    ):
        runtime.kv_num_blocks_buffer = torch.empty(
            1, num_heads, output_q_blocks, dtype=torch.int32, device=q_real.device
        )
