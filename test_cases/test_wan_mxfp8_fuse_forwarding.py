...
class WanNvfp4SplitNStrideTest(unittest.TestCase):
    ...
    def test_gelu_operator_forwards_complete_tensors(self):
        ...
        values = {name: object() for name in ("input", "input_quant", "input_scale", "weight", "weight_scale", "alpha", "bias", "output")}
        operator.act_quant_func = lambda value: (values["input_quant"], values["input_scale"])
        operator.weight = values["weight"]
        operator.weight_scale = values["weight_scale"]
        operator.alpha = values["alpha"]
    ...
    def test_thor_incompatibilities(self):
        invalid_cases = [
        ...
        for extra_config, message in invalid_cases:
            with self.subTest(extra_config=extra_config):
                with self.assertRaisesRegex(NotImplementedError, message):
                    self.make_ffn(thor=True, **extra_config)
...
class WanMxfp8FuseForwardingTest(unittest.TestCase):
    ...
    def test_nvfp4_ffn0_gelu_fusion_dispatches_once(self):
        ...
        phase.ffn_2 = SimpleNamespace(
            apply=lambda value: (_ for _ in ()).throw(RuntimeError("ordinary FFN2 must be skipped")),
            apply_residual_gate=lambda y, residual, gate: residual.copy_(y),
        )
        ...
        self.assertTrue(torch.equal(x, torch.full_like(x, 3.0)))
