...
class HunyuanImage3PhaseExecutionTests(unittest.TestCase):
    ...
    def test_flashinfer_multi_micro_ar_still_uses_one_official_call(self):
        ...
        moe = SimpleNamespace(
            ...
            active_flashinfer_multi_micro_weights=lambda _device, _dtype: (_ for _ in ()).throw(
                AssertionError("AR must not request the denoise multi-micro pack")
            ),
        )
        official_calls = []
    ...
    def test_flashinfer_multi_micro_denoise_uses_custom_once_and_allreduces_once(self):
        ...
        moe = SimpleNamespace(
            ...
            active_flashinfer_weight_shards=lambda _device, _dtype: (_ for _ in ()).throw(
                AssertionError("fused denoise must not enter the legacy shard loop")
            ),
        )
        custom_calls = []
