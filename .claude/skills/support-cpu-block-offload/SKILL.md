...
description: 为 LightX2V 模型接入、审查和调试 CPU block offload 与多进程 CPU 权重共享（shared_cpu_weights）。适用于新增模型适配器、接通共享权重加载与 GPU 缓冲调度、处理共享存储及异步复制生命周期，并交付 scripts 模型目录下 offload 子目录中支持 host／NUMA 切换的启动脚本、配置和验证记录。
---
...
每个模型在 `scripts/<model>/offload/` 保留一个共享 offload 启动脚本、一个 README，并引用一份纳入版本管理的 JSON 配置。脚本和共享配置的文件名不含任务名；通过 `TASK` 选择模型已支持的任务。默认示例为 H3 的 `t2av`、Qwen／Hunyuan 的 `t2i`、Wan 的 `i2v`：
...
配置优先放 `configs/<model>/offload/`；Wan 沿用 `configs/offload/block/`。默认 host + 8 卡，`CUDA_VISIBLE_DEVICES` 未设置时使用 `0,1,2,3,4,5,6,7`，设置后按模型支持的并行拓扑使用指定显卡。通过 `SHARED_CPU_WEIGHT_SCOPE=host|numa` 切换共享方式，不再按 scope、卡数或任务复制共享启动文件；任务分支传入各自所需的图片、尾帧、视频或音频参数，并区分任务相关的缓存和输出。共享入口目录只交付共享脚本和对应配置，不保留单独的私有 CPU block 基线脚本或专用配置。

默认共享配置包含 `cpu_offload=true`、`offload_granularity="block"`、`shared_cpu_weights=true`、`shared_cpu_weight_backend="sysv"`、`shared_cpu_weight_scope="host"`、`shared_cpu_weight_strict_numa=true`、`shared_cpu_weight_register_chunk_mb=128`、`lazy_load=false`。额外组件仅开启已接通的对应共享开关。

脚本和配置还须满足：

- 同一任务切换 host／NUMA 时使用同一 checkpoint、task、输入、shape、steps、seed、dtype、算子和并行规模，切换仅改变共享 scope。
- 根据脚本位置定位仓库，引用 `scripts/base/base.sh` 时遵循变量契约；用当前环境的 `python -m` 启动，并支持模型路径、输入输出和可见设备覆盖。
- 使用默认配置时，可在独立临时 JSON 中调整 scope 和并行规模；退出时清理，源文件保持不变。进程数须等于有效配置中的 `tensor_p_size * cfg_p_size * seq_p_size`。
- 显式指定完整配置时保留其拓扑，核对可见卡数；scope 环境变量若设置则覆盖配置，否则沿用配置值。不要静默覆盖用户指定的并行参数。
- 显式 NUMA 模式不能静默退回 host；未知拓扑、严格 NUMA 绑定失败应报错。可见卡数仍受模型注意力头和并行实现限制。
- README 写清默认任务、`TASK` 支持范围及输入要求、NUMA 切换、其他卡数、配置位置、参数默认值与优先级、环境及资产要求、组件共享范围。使用说明中不堆放实验报告。
...
3. `scripts/<model>/offload/` 下支持 host／NUMA 的统一脚本、一份共享配置和 README。
